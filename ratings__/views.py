...
def get_ratings_view2(request):



	#object_id = Shop.objects.get(slug="test-ftrina").id
	object_id = Shop.objects.get(slug="lilygifts").id
	...
	object_id = Product.objects.get(pk="5e1e5c15-da08-48d1-9509-35f221817693").id
...
def get_ratings_view(request):
	...
	rating_for = Rating.objects.for_instance(instance).average
	print rating_for
	rating_for = 4
