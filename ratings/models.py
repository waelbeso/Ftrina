...
class RatingManager(models.Manager):
    def for_instance(self, instance):
        ...
        ratings, created = self.get_or_create(content_type=content_type, object_id=instance.pk)
        return ratings
    ...
    def rate(self, instance, score, user=None, ip=None):
        ...
        if existing_rating:
            ...
            rating, created = self.get_or_create(content_type=content_type, object_id=instance.pk)
...
class Rating(models.Model):
    ...
    object_id = models.UUIDField(primary_key=False, editable=False,default=uuid.uuid4)
    ...
    def __str__(self):
        return '{} <{}>'.format(self.content_object,self.total)
...
class UserRatingManager(models.Manager):
    def for_instance_by_user(self, instance, user=None):
        ...
        user = _clean_user(user)
        if user:
            return self.filter(rating__content_type=content_type, rating__object_id=instance.pk, user=user).first()
        else:
            return None
